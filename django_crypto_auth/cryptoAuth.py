...
try:
    from .app_settings import CryptoAuthSettings, crypto_auth_default_setting
except:
...
class CryptoAuthentication(CryptoAuthSettings):
    ...
    def token_key(
        self, key: str, encrypt: bool = False, subset: bool = False, shuffle: list = []
    ) -> str:
        if encrypt:
            return self._encrypt_key(
                self._key_int_to_letters(key[0 : -1 : self.subset_length]), shuffle
            )
        if subset:
    ...
    def _key_int_to_letters(self, key: str) -> str:
        return "".join([chr(int(i) + 65).lower() if i.isdigit() else i for i in key])
